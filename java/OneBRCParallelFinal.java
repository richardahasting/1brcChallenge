...
/**
 * 1 Billion Row Challenge - Final Optimized Java Implementation
 *
 * Ultimate optimizations:
 * - Memory-mapped I/O for zero-copy access
 * - Parallel chunk processing (no sequential reader bottleneck)
 * - buffer.duplicate() for fast sequential reads
 * - Byte array keys (NO String creation during processing!)
 * - Custom ByteArrayKey with efficient hashing
 * - Strings created only at final merge (100× vs 1 billion×)
 *
 * This eliminates the last major allocation overhead!
 *
 * Usage:
 *   javac OneBRCParallelFinal.java
 *   java OneBRCParallelFinal <input_file> [num_threads] [chunk_size_kb]
 */
public class OneBRCParallelFinal {
    ...
    // ============================================================
    // Byte Array Key (avoids String creation!)
    // ============================================================

    /**
     * Wrapper for byte array to use as HashMap key.
     *
     * Key optimization: Store byte arrays directly during processing,
     * only convert to String at the very end when merging results.
     *
     * This eliminates ~1 BILLION String allocations!
     */
    static class ByteArrayKey {
        private final byte[] bytes;
        private final int hash;

        ByteArrayKey(byte[] bytes, int length) {
        ...
        @Override
        public int hashCode() {
            return hash;  // Cached for performance
        }
    ...
    public static void main(String[] args) {
        if (args.length < 1) {
            System.err.println("Usage: java OneBRCParallelFinal <input_file> [num_threads] [chunk_size_kb]");
            System.err.println();
        ...
        System.err.println("=== 1 Billion Row Challenge - Final Optimized ===");
        ...
        System.err.println();
        System.err.println("Optimizations: buffer.duplicate() + byte array keys (no String allocations!)");
        System.err.println();
    ...
    private static Map<String, Stats> processFile(String filename, int numThreads, int chunkSizeKB)
        ...
        try (RandomAccessFile file = new RandomAccessFile(filename, "r");
            ...
            List<Callable<Map<ByteArrayKey, Stats>>> tasks = new ArrayList<>();
            for (WorkChunk chunk : chunks) {
                tasks.add(() -> processChunk(chunk));
            }

            // Execute all tasks in parallel and wait for completion
            List<Future<Map<ByteArrayKey, Stats>>> futures = executor.invokeAll(tasks);

            // Merge results from all workers
            // This is where we convert ByteArrayKey to String (only ~100 times!)
            Map<String, Stats> merged = new HashMap<>(16384);
            for (Future<Map<ByteArrayKey, Stats>> future : futures) {
                Map<ByteArrayKey, Stats> workerResult = future.get();

                for (Map.Entry<ByteArrayKey, Stats> entry : workerResult.entrySet()) {
                    String stationName = entry.getKey().toStringUTF8();  // Convert to String ONLY here
                    merged.merge(stationName, entry.getValue(), (existing, newStats) -> {
    ...
    /**
     * Processes a single chunk using byte array keys.
     *
     * NO String allocations during processing!
     */
    private static Map<ByteArrayKey, Stats> processChunk(WorkChunk chunk) {
        Map<ByteArrayKey, Stats> localStats = new HashMap<>(4096);
        ...
        while (buffer.hasRemaining()) {
            ...
            // Update statistics using ByteArrayKey (no String allocation!)
            ByteArrayKey key = new ByteArrayKey(stationBytes, stationLen);
            localStats.computeIfAbsent(key, k -> new Stats()).update(temperature);
        }
