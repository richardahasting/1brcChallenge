...
public class OneBRCParallelUltimate {
    ...
    static class StationTable {
        ...
        private final int[] mins;          // Min temperatures (×10, as integers!)
        private final int[] maxs;          // Max temperatures (×10, as integers!)
        private final long[] sums;         // Sum of temperatures (×10, as integers!)
        ...
        StationTable() {
            ...
            mins = new int[TABLE_SIZE];
            maxs = new int[TABLE_SIZE];
            sums = new long[TABLE_SIZE];
         ...
         *
         * Temperature is stored as integer (×10), e.g., 23.5 → 235
         */
        void update(byte[] nameBytes, int nameLength, int temperatureTimes10) {
            ...
            while (occupied[index]) {
                ...
                if (lengths[index] == nameLength &&
                    ...
                    // Found it - update stats (pure integer arithmetic!)
                    if (temperatureTimes10 < mins[index]) mins[index] = temperatureTimes10;
                    if (temperatureTimes10 > maxs[index]) maxs[index] = temperatureTimes10;
                    sums[index] += temperatureTimes10;
                    counts[index]++;
                    return;
                }
            ...
            mins[index] = temperatureTimes10;
            maxs[index] = temperatureTimes10;
            sums[index] = temperatureTimes10;
            counts[index] = 1;
            occupied[index] = true;
        }
         ...
         * Divides by 10 to convert integers back to doubles.
         */
        Map<String, Stats> toMap() {
            ...
            for (int i = 0; i < TABLE_SIZE; i++) {
                if (occupied[i]) {
                    ...
                    // Convert back to doubles by dividing by 10
                    stats.min = mins[i] / 10.0;
                    stats.max = maxs[i] / 10.0;
                    stats.sum = sums[i] / 10.0;
    ...
    private static StationTable processChunk(WorkChunk chunk) {
        ...
        while (buffer.hasRemaining()) {
            ...
            // Parse temperature as integer (×10)
            int temperatureTimes10 = parseTemperatureInt(buffer);

            // Update custom hash table (NO allocations, pure integer arithmetic!)
            table.update(stationBytes, stationLen, temperatureTimes10);
        }
    ...
    // ============================================================
    // Temperature parsing (custom, optimized - INTEGER version!)
    // ============================================================
     ...
     *   "99.9\n" → 999
     *
     * This uses ONLY integer arithmetic - faster than floating point!
     */
    private static int parseTemperatureInt(MappedByteBuffer buffer) {
        ...
        // Combine: value * 10 + decimal (e.g., 23 * 10 + 5 = 235)
        int result = value * 10 + decimal;
